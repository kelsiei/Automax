...
{
    private readonly ILiteCollection<ServiceRecord> _collection;

    public LiteDbServiceRecordDataAccess(LiteDBHelper dbHelper)
    {
        _collection = dbHelper.GetCollection<ServiceRecord>("service_records");
    }
    ...
    public Task<int> SaveServiceRecordAsync(ServiceRecord record)
    {
        if (record.Id == 0)
        {
    ...
    public Task DeleteServiceRecordAsync(int id)
    {
