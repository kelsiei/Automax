...
public class ReminderLogicTests
{
    ...
    public async Task GetDateBasedRemindersForUserAsync_IncludesOnlyAccessibleVehicles()
    {
        ...
        vehicleData.Setup(v => v.GetVehiclesAsync(It.IsAny<int>(), false, It.IsAny<IEnumerable<int>?>()))
            .ReturnsAsync((int _, bool _, IEnumerable<int>? allowed) =>
            {
                var allowedSet = new HashSet<int>(allowed ?? Enumerable.Empty<int>());
                return vehicles.Where(veh => allowedSet.Contains(veh.Id)).ToList();
            });
        vehicleData.Setup(v => v.GetVehiclesAsync(It.IsAny<int>(), true, It.IsAny<IEnumerable<int>?>()))
    ...
    public async Task GetDateBasedRemindersForUserAsync_IncludesAnyMetricWithDueDate()
    {
        ...
        var vehicleData = new Mock<IVehicleDataAccess>();
        vehicleData.Setup(v => v.GetVehiclesAsync(It.IsAny<int>(), It.IsAny<bool>(), It.IsAny<IEnumerable<int>?>())).ReturnsAsync(new List<Vehicle>
        {
            new() { Id = 1, Year = 2020, Make = "A", Model = "A", LicensePlate = "A" }
        });
    ...
    public async Task GetDateBasedRemindersForUserAsync_MapsFieldsCorrectly()
    {
        ...
        var vehicleData = new Mock<IVehicleDataAccess>();
        vehicleData.Setup(v => v.GetVehiclesAsync(It.IsAny<int>(), It.IsAny<bool>(), It.IsAny<IEnumerable<int>?>())).ReturnsAsync(new List<Vehicle> { vehicle });
